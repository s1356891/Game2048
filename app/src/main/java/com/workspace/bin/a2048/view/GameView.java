package com.workspace.bin.a2048.view;
...
import com.workspace.bin.a2048.GameActivity;
import com.workspace.bin.a2048.app.MyApplication;
import com.workspace.bin.a2048.bean.GameItem;
...
public class GameView extends GridLayout implements View.OnTouchListener {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isMoved()) {
                    addRandomNum();
                    // 修改显示分数
                    GameActivity.getGameActivity().setScore( MyApplication.SCORE, 0);
                }
                checkCompleted();
                break;
            default:
    ...
    private void saveHistoryMatrix() {
        mScoreHistory = MyApplication.SCORE;
        for (int i = 0; i < mGameLines; i++) {
            for (int j = 0; j < mGameLines; j++) {
...
//        }
    ...
    private void checkCompleted() {
        ...
        if (result == 0) {
            if ( MyApplication.SCORE > mHighScore) {
                SharedPreferences.Editor editor = MyApplication.mSp.edit();
                editor.putInt(MyApplication.KEY_HIGH_SCORE,  MyApplication.SCORE);
                editor.apply();
                GameActivity.getGameActivity().setScore( MyApplication.SCORE, 1);
                MyApplication.SCORE = 0;
            }
                    ...
                    .setNegativeButton("Continue",
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface arg0,
                                    ...
                                    SharedPreferences.Editor editor = MyApplication.mSp.edit();
                                   if (mTarget == 2048) {
                                        editor.putInt(MyApplication.KEY_GAME_GOAL, 4096);
                                        mTarget = 4096;
                                        GameActivity.getGameActivity().setGoal(4096);
                                    } else {
                                        editor.putInt(MyApplication.KEY_GAME_GOAL, 8192);
                                        mTarget = 4096;
